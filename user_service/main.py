...
async def create_user(user_dto: UserCreateDTO, db: AsyncSession = Depends(get_db)):
    ...
    API_REQUEST_DURATION.labels(method="post", endpoint="/users").observe(
...
async def get_user(
    ...
    # Метрики
    API_REQUEST_DURATION.labels(method="get", endpoint="/users/{id}").observe(
        time.time() - start_time
    )
...
async def get_all_users(db: AsyncSession = Depends(get_db)):
    ...
    API_REQUEST_DURATION.labels(method="get", endpoint="/users").observe(
        time.time() - start_time
    )
...
if __name__ == "__main__":
    uvicorn.run(
        "user_service.main:app", host="0.0.0.0", port=settings.server_port, reload=True
    )
