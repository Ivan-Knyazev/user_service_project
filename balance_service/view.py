...
class CustomPostgresWalletView(PostgresTrackingRecorder):
    ...
    def credit_balance(
        self, wallet_id: str, amount: float, timestamp: datetime, tracking
    ):
        with self.Session() as session:
            with session.begin():
                ...
                self._save_history_entry(
                    session, wallet_id, "CREDIT", amount, timestamp
                )
                self.insert_tracking(tracking)
    ...
    def _save_history_entry(
        ...
        history_entry = WalletHistoryView(
            wallet_id=wallet_id, operation=operation, amount=amount, timestamp=timestamp
        )
