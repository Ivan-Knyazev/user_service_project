...
@app.get(
    ...
    response_model=list[BalanceHistoryDTO],
    tags=["Query"],
)
def get_history(wallet_id: uuid.UUID):
    with ReadSessionLocal() as session:
        history = (
            session.query(WalletHistoryView)
            .filter(WalletHistoryView.wallet_id == str(wallet_id))
            .order_by(WalletHistoryView.timestamp.desc())
            .all()
        )

        return [
            BalanceHistoryDTO(
                operation=h.operation, amount=h.amount, timestamp=h.timestamp
            )
            for h in history
        ]
    ...
    with ReadSessionLocal() as session:
        ...
        for table in tables_to_truncate:
            try:
                ...
                session.rollback()
                print(
                    f"Таблица {table} еще не создана или пуста. Пропускаем. Ошибка: {e}"
                )
